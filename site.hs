...
staticCtx hashes =
    filepathCtx
        `mappend` genericStringField "githash" hashes
        `mappend` defaultContext

postCtx :: Tags -> [(Identifier, String)] -> Context String
postCtx tags times =
    genericStringField "lastModified" times
...
gitModTime = ShlCommand
    { cmname = "git"
    , cmargs = ["log", "-1", "--format=%ad", "--date=format:%b %d, %Y"]
    }

gitShortHash :: ShlCommand
gitShortHash =
    ShlCommand {cmname = "git", cmargs = ["log", "-1", "--pretty=format:%h"]}
